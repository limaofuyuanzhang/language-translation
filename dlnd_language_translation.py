...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for source in sources:
        ...
        for word in source.split():
            word_id = source_vocab_to_int[word]
            source_to_int.append(word_id)
        source_id_text.append(source_to_int)
    ...
    for target in targets:
        ...
        for word in target.split():
            word_id = target_vocab_to_int[word]
            target_to_int.append(word_id)
        target_to_int.append(eos)
        target_id_text.append(target_to_int)
...
def process_decoder_input(target_data, target_vocab_to_int, batch_size):
    ...
    vector = tf.fill([batch_size, 1], target_vocab_to_int['<GO>'])
    preprocessed_target_data = tf.concat([vector, ending], 1)
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    for _ in range(num_layers):
        ...
        drop_wrapped_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=keep_prob)
        cells.append(drop_wrapped_cell)
    
    rnn_output, rnn_state = tf.nn.dynamic_rnn(tf.contrib.rnn.MultiRNNCell(cells), 
                                              embeddings, 
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, 
    ...
    """
    # TODO: Implement Function
    training_helper = seq2seq.TrainingHelper(dec_embed_input, sequence_length=target_sequence_length)
...
def decoding_layer(dec_input, encoder_state,
    ...
    decode_embed_input = tf.nn.embedding_lookup(decode_embeddings, dec_input)
    
    cells = []
    for _ in range(num_layers):
        lstm_cell = LSTMCell(rnn_size, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=24))
        drop_wrapped_cell = DropoutWrapper(lstm_cell, output_keep_prob=keep_prob)
        cells.append(drop_wrapped_cell)

    decode_cell = MultiRNNCell(cells)
    ...
    with tf.variable_scope("decode", reuse=True):
        tgo = target_vocab_to_int['<GO>']
        teos = target_vocab_to_int['<EOS>']
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size,
    ...
    training_decoder_output, inference_decoder_output = decoding_layer(decoder_input, encoding_state,
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    word_ids = [vocab_to_int.get(word, vocab_to_int.get('<UNK>')) for word in words]
    return word_ids
